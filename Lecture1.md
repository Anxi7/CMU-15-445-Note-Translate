...
在数据库中，*data model*(数据模型)是用来描述数据的概念集合(描述数据、数据联系、数据语义及一致性约束)。例如：关系型(最常见), NoSQL(键值对、图), 数组/矩阵/向量(对于机器学习而言)等。  

*Schema*(模式)是基于data model的对特定数据集合的描述

### 常见的数据模型

- 关系Relational（大多数数据库管理系统）
- 键值对（NoSQL）
- 图（NoSQL）
- 文件/XML/对象（NoSQL）
- 宽列/列族(Wide-Column/Column-family)（NoSQL）
- 数组/矩阵/向量（机器学习）
- 分层（已过时/传统/少见）
- 网络（已过时/传统/少见）
- 多值（已过时/传统/少见）

"Obsolete/Legacy/Rare"意味着该数据模型已经过时、传统或者在现代数据库系统中使用频率较低。

> XML（Extensive Markup Language, 可扩展标记语言）是一种用于**描述数据的标记语言**。它允许用户定义自己的标记，从而更好地组织和呈现数据。XML的设计目标是传输和存储数据，同时保持数据的可读性和可扩展性。
>
> XML被广泛用于在不同系统之间传输和共享数据，特别是在网络上。它是许多互联网协议和标准的基础，如SOAP（简单对象访问协议）和RSS（聚合站点内容）。XML也常用于配置文件和数据存储中，因为它的结构化特性使得数据更易于管理和解析。
>
> ---
>
> Wide-column存储或者称为Column-family存储是一种基于列的数据库模型，通常用于处理具有大量列的数据。它们是NoSQL数据库的一种类型，适用于需要处理大量数据以及需要水平扩展性的应用场景。
>
> 在Wide-column/Column-family模型中，数据被组织成列的集合，而不是传统的行。每个列族（Column Family）可以包含大量的列，每个列都有一个唯一的名称和相应的值。这种模型允许每行具有不同的列集合，而不需要遵循固定的表结构。此外，列族还可以按照需要动态添加新的列，而不会影响已有的数据。
>
> 这种模型的设计使得它特别适用于需要存储大量数据并进行高效的读写操作的场景，例如日志存储、分析数据以及内容管理系统等。同时，由于Wide-column/Column-family数据库通常支持水平扩展，因此它们能够处理非常大规模的数据集，并具有较好的性能和可扩展性。Apache HBase和Apache Cassandra是两个常见的Wide-column/Column-family数据库系统的例子。
...
从数学意义上而言，集合中的元素应该是互异的，但在实际的数据库系统中，一般允许重复的元素存在，原因之一是去重消耗过大。
...
一个有n个属性的关系被称为n元关系。在本课程中，我们会交替使用关系和表这两个术语。一个n元关系和一个有n列的表是等价的。

### 码

一个关系的主码(*primary key*)唯一标识了一个元组。如果没有定义内部主码，某些DBMS会自动创建一个。许多DBMS支持自动生成主码，因此应用程序不需要手动设置主码，但是对于某些DBMS而言，主码仍是必需的。

外码(*foreign key*)规定一个关系中的一个属性必须映射到另一个关系中的一个元组上（也就是说，引用关系中的属性取值必须在被引用关系中存在）。例如，我们可以在专辑这张表中加入艺人id这一属性（从专辑引用艺人的外码）
...
约束是用户自定义的条件，它必须适用于数据库的任何实例。

## 5. 数据操作语言DMLs

> Data Manipulation Languages是一门在数据库中存储和检索信息的语言。

- 过程式(Procedural)：查询指定了DBMS应该用来查找所需结果的(高级)策略。即指定了什么样的数据是需要的，以及如何来获取这些数据。例如，使用for循环来扫描所有记录并且统计扫描到的记录数，以检索表中的记录总数。（其中指明了需要的数据 —— 表中的记录总数，以及数据的获取方式 —— 使用for循环扫描整个表格，但貌似没有体现base on sets / bags这一说法）

> 原Note中使用了**base on sets / bags**这一描述，在这里，我将其理解为指定查询结果是否需要满足互异性。
>
> Sets: 集合，其中每个元素都是唯一的，没有重复项；Bags: 袋，也称为多重集合，其允许重复元素。
>
> 基于两种不同的数据结构，DBMS在执行查询时可能会采取不同的策略来处理查询结果，即是否保留重复的元素或者保证结果的唯一性。

- 非过程式(描述式)：查询只指定什么样的数据是需要的，但不说明如何获取它们。例如，使用SQL语句 select count(*) from artist来统计艺人表中有多少条记录。

## 6. 关系代数

关系代数是在关系中检索和操作元组的一组基本运算。每个运算符都接受一个或多个个关系作为输入，并返回一个新的关系。为了编写查询语句，我们可以将这些操作符“链接”在一起，以创造更多复杂的运算。

### Select选择

Select接受一个关系作为输入，并且返回这个关系的满足选择谓词的子集。谓词的作用就像一个过滤器，我们可以使用合取(conjunctions)和析取(disjunctions)将多个谓词连接起来。（合取和析取是离散数学中的概念，即与和或）

语法：$\sigma_{predicate}(R)$

### Projection投影

Project接受一个关系作为输入，并且返回一个元组中仅包含指定属性的关系。你可以重排输入关系中的属性顺序，也可以操作这些值（可以对属性使用算术表达式，执行一些简单的加减乘除运算）

语法：$\Pi_{A1, A2, ..., An}(R)$  
示例：$\Pi_{b\_id, a\_id}(\sigma_{a\_id = 'a2'}(R))$

### Union并

Union接受两个关系作为输入，并返回一个包含所有满足下面条件的元组的关系：该元组至少包含在两个输入关系之一里面。

语法：($R \cup S$)  
SQL语句：`(SELECT * FROM R) UNION ALL (SELECT * FROM S)`  
注意：上述SQL语句中有一个ALL，指的是保留集合中的重复元素，一般情况下，SQL中的集合运算是去重的。
